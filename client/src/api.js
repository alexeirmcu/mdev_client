...
async function request(method, path, body) {
  ...
  if (!res.ok) {
    ...
    try {
      ...
      msg = Array.isArray(err) ? err.map((e) => e.description).join("; ") : err.title || msg;
    } catch {}
  ...
  if (res.status === 204) {
    console.log(`[API] << 204 ${method} ${url}`);
    return null;
  }
...
export function getTrips(cityCode, startDate, endDate) {
  ...
  return request("GET", `/trips${qs ? `?${qs}` : ""}`);
}
...
export function toggleCompletion(tripId, dayIndex, placeId, isCompleted) {
  return request("PATCH", `/trips/${tripId}/days/${dayIndex}/completion`, {
    placeId,
    isCompleted,
  });
}
...
export function replanTrip(tripId, currentDateTime, scope, currentBlockWeather) {
  return request("POST", `/trips/${tripId}/replan`, {
    currentDateTime,
    scope,
    currentBlockWeather,
  });
